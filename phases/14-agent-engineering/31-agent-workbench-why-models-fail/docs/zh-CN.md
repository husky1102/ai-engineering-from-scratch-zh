...
> 一个能力强的模型还不够。可靠智能体需要一个工作台：指令、状态、范围、反馈、验证、评审和交接。把这些拿掉，即使是前沿模型也会产出不能安全发布的工作。

**类型:** 学习 + 构建
**语言:** Python (stdlib)
**先修:** 第 14 阶段第 01 课（Agent Loop），第 14 阶段第 26 课（Failure Modes）
**时间:** ~45 分钟

## 学习目标

- 区分模型能力与执行可靠性。
- 说出决定智能体是否能交付的七个工作台表面。
- 在一个小型 repo 任务上比较“只靠提示”的运行与“工作台引导”的运行。
- 生成一份失败模式报告，把每个缺失表面映射到它造成的症状。

## 要解决的问题

你把一个前沿模型放进真实 repo，要求它添加输入验证。它打开四个文件，写出看似合理的代码，声明成功，然后停止。你运行 tests。两个失败。第三个被改动的文件与验证毫无关系。没有记录说明智能体假设了什么、先尝试了什么、还剩什么要做。

模型并不是不懂 Python。它不懂的是这项工作。它不知道什么算完成、哪里允许写、哪些 tests 是权威的，或者下一次 session 应该如何接手。

这不是模型 bug。这是工作台 bug。智能体周围缺失了那些把一次性生成变成可靠、可恢复工程工作的表面。
...
| 表面 | 它承载什么 | 缺失时的失败 |
|------|------------|--------------|
| 指令 | 启动规则、禁止动作、完成定义 | 智能体猜测什么算可交付 |
| 状态 | 当前任务、已触碰文件、阻塞项、下一步动作 | 每个 session 都从零开始 |
| 范围 | 允许文件、禁止文件、验收标准 | 修改泄漏到无关代码 |
| 反馈 | 捕获进循环的真实命令输出 | 智能体在 HTTP 400 上声明成功 |
| 验证 | Tests、lint、smoke run、scope check | “看起来不错”进入 main |
| 评审 | 由不同角色进行第二遍检查 | Builder 给自己的作业打分 |
| 交接 | 改了什么、为什么、还剩什么 | 下一个 session 重新发现一切 |
...
循环闭合在 state file 上，而不是 chat history 上。Chat 是易失的。Repo 才是记录系统。

### 工作台 versus prompt engineering

Prompting 告诉模型这一轮你想要什么。工作台告诉模型如何跨轮次、跨 sessions 地做工作。大多数智能体失败故事，本质上是披着 prompt-engineering 外衣的工作台失败。

### 工作台 versus framework

Framework 给你 runtime（LangGraph、AutoGen、Agents SDK）。工作台给智能体一个在该 runtime 内工作的地方。两者都需要。这个 mini-track 讲的是第二个。
...
先把智能体标签拿掉片刻。一次 agent run 是跨越时间、进程和机器的 computation。要让它可靠，你需要任何生产系统都需要的同一批原语。

| 原语 | 它是什么 | 它为智能体承载什么 |
|------|----------|---------------------|
| Function | 类型化 handler。尽可能纯。拥有自己的输入和输出。 | 一次 tool call、一次规则检查、一个验证步骤、一次 model invocation |
...
- **指令** — policy + function metadata。Rules 是 checks（functions）。Router（`AGENTS.md`）是附着在 runtime startup 上的 policy。
- **状态** — session persistence。Runtime 每一步都会读取的 keyed store。File、KV 或 DB；重要的是 persistence semantics，不是 storage backend。
- **范围** — 每个 task 的 authorization policy。Allowed/forbidden globs 是 ACL。需要 approvals 是 permission lattice。
- **反馈** — 写入 queue 的 invocation log。每次 shell call 都是一条 record，durable、replayable。
- **验证** — 一个 function。对 inputs 确定性。由 task close 触发。Fails closed。
- **评审** — 一个单独 worker，对 builder artifacts 只有 read-only authz，对 review reports 只有 write-only authz。
- **交接** — 由 session-end trigger 发出的 durable record。下一个 session 的 startup trigger 读取它。
...
| Vendor 或社区模式 | 它实际是什么 |
|-------------------|--------------|
| Ralph Loop（Claude Code、Codex、agentic_harness book）— 当 agent 过早尝试停止时，将原始意图重新注入到 fresh context window | 一个用干净上下文重新 enqueue task 的 trigger；session persistence 将 goal 向前承载 |
...
`code/main.py` 把一个 tiny repo task 运行两次。第一次只靠提示，第二次接入七个表面。相同模型，相同任务。脚本统计失败运行缺失了哪些表面，并打印 failure-mode report。

Repo task 刻意保持很小：给一个单文件 FastAPI-style handler 添加输入验证，并写出一个 passing test。
...
输出：两次运行的并排 log、一个总结只靠提示运行的 `failure_modes.json`，以及工作台运行的一行 verdict。

这个 agent 是很小的 rule-based stub；重点是表面，不是模型。在这个 mini-track 的其余部分，你会把每个 surface 重建成真实、可复用 artifact。

## 实际使用

真实项目里已经存在三类工作台表面，哪怕没人这样称呼它们：

- **Claude Code、Codex、Cursor。** `AGENTS.md` 和 `CLAUDE.md` 是指令表面。Slash commands 是范围。Hooks 是验证。
- **LangGraph、OpenAI Agents SDK。** Checkpoints 和 session stores 是状态表面。Handoffs 是交接表面。
- **真实 repo 上的 CI。** Tests、lint 和 type-check 是验证。PR template 是交接。CODEOWNERS 是评审。
...
1. 选择一个你已经运行 agent 的 repo。把七个表面从 0（缺失）到 2（健康）打分。你最弱的 surface 是什么？
2. 扩展 `main.py`，让只靠提示的运行也产出一个假的 “success” claim。验证 gate 应该能捕捉它。
3. 为你自己的产品添加第八个 surface。说明为什么它不能被归并到现有七个之一。
4. 用另一个会 hallucinate 额外 file write 的 stub agent 重新运行脚本。哪个 surface 最先捕捉它？
5. 将第 14 阶段第 26 课的五种行业反复出现失败模式映射到七个表面。每个 surface 设计来吸收哪种 mode？

## 关键术语

| 术语 | 人们通常怎么说 | 它实际意味着什么 |
|------|----------------|------------------|
| Workbench | “那套配置” | 模型周围让工作可靠的工程化表面 |
| Surface | “一个文档”或“一个脚本” | Agent 每轮读取或写入的命名、machine-readable input |
| System of record | “那些笔记” | Chat history 消失时 agent 视为 truth 的文件 |
| Definition of done | “验收” | 一个 objective、file-backed checklist，agent 不能伪造 |
| Workbench audit | “Repo readiness check” | 对七个表面的一轮检查，能在工作开始前标出缺失部分 |
...
- Phase 14 · 26 — failure modes catalog，七个表面设计来吸收它们
