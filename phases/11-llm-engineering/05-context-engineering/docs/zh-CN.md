...
- 计算所有 context window 组件的 token budget，包括 system prompt、tools、history、retrieved docs 和 generation headroom
- 实现 context window 管理策略：conversation history 的 truncation、summarization 和 sliding window
- 对 context 组件进行优先级排序和顺序安排，让模型 attention 尽可能集中在最相关信息上
- 构建 context assembler，根据 query type 和可用 window space 动态分配 token
...
你放进窗口的每个 token，都会挤掉一个本可以承载更相关信息的 token。每个无关 tool definition、每个 stale conversation turn、每个无法回答问题的 retrieved text chunk，都会让模型在任务上稍微变差。

## 核心概念

### Context Window 是稀缺资源

把 context window 当作 RAM，而不是 disk。它速度快，可以被直接访问，但有限。你无法塞进所有东西，必须选择。
...
**System prompt**：设置 persona、constraints 和 behavioral rules。它放在最前面，并跨 turn 保持不变。Claude Code 的 system prompt 大约使用 6,000 tokens，包含 tool definitions 和 behavioral instructions。保持紧凑。system prompt 中的每个 word 都会在每次 API call 中重复。
...
**Retrieved context**：来自 vector database 的 documents、search results、file contents。retrieval 质量直接决定 response 质量。坏 retrieval 比没有 retrieval 更糟，因为它会用噪声填满窗口，并主动误导模型。
...
```mermaid
graph TD
    subgraph Memory["Memory 架构"]
        direction TB
        STM["Short-term Memory（短期记忆）\n(current conversation)\n直接进入 context window"]
        LTM["Long-term Memory（长期记忆）\n(facts, preferences)\nDB -> session start 时 retrieval"]
        EM["Episodic Memory（情节记忆）\n(past interactions)\nEmbeddings -> similarity retrieval"]
    end

    Q["Current Query（当前查询）"] --> STM
    Q --> LTM
    Q --> EM
...
关键洞见：不同 query 需要不同 context。static system prompt + static tools + static history 很浪费。最好的系统会按 query 动态 assemble context。
...
### Step 4: Conversation History 压缩器
...
Claude Code 用 layered approach 管理 context。system prompt 包含 behavioral rules 和 tool definitions（约 6K tokens）。当你打开文件时，它的 contents 会作为 context 注入。当你搜索时，results 会加入。Old conversation turns 会被 summarized。CLAUDE.md 提供跨 sessions 持久化的 long-term memory。
...
1. 给 `ContextBudget` class 添加 “token waste detector”。它应标记占用超过 30% budget 的 component，并为每类 component 提供具体 compression strategy（summarize history、prune tools、re-rank documents）。
...
| 术语 | 常见说法 | 实际含义 |
|------|----------|----------|
| Context window | “模型能读多少” | 模型在单次 forward pass 中处理的最大 tokens 数（input + output）——GPT-5 为 400K，Claude Opus 4.7 为 200K（1M beta），Gemini 3 Pro 为 2M |
...
| Lost-in-the-middle | “模型会忘掉中间内容” | 实证发现：LLM 对 context 开头和结尾 attention 更好，中间信息会有 10-20% accuracy drop |
| Token budget | “还剩多少 token” | 对 context window capacity 在 components（system prompt、tools、history、retrieval、generation）之间的显式分配，并设置 per-component limit |
| Dynamic context | “按需加载内容” | 基于 intent classification、relevant tool selection 和 retrieval results，为每个 query 以不同方式 assemble context window |
| History summarization | “压缩 conversation” | 用 concise summary 替代逐字 old conversation turns，在保留 key information 的同时降低 token cost |
| Tool pruning | “只包含相关 tools” | classify query intent，只包含匹配的 tool definitions，可将 tool token cost 降低 60-80% |
| Long-term memory | “跨 session 记住东西” | 存在 database 中并在 session start retrieve 的 facts 和 preferences，例如 CLAUDE.md、ChatGPT Memory 与类似系统 |
| Episodic memory | “记住具体过去事件” | 把 past interactions 存为 embeddings，并在 current query 与过去 conversation 相似时 retrieve |
| Generation budget | “给答案留空间” | 为模型 output 预留的 tokens。如果 context 完全填满 window，模型就没有空间响应 |

## 延伸阅读

- [Liu et al., 2023 -- "Lost in the Middle: How Language Models Use Long Contexts"](https://arxiv.org/abs/2307.03172)：关于 position-dependent attention 的权威研究，展示模型难以处理 long context 中间的信息
...
- [Ainslie et al., "GQA: Training Generalized Multi-Query Transformer Models from Multi-Head Checkpoints" (EMNLP 2023)](https://arxiv.org/abs/2305.13245)：grouped-query attention 论文，在 production decoders 中把 KV memory 降低 8x，且无质量损失
