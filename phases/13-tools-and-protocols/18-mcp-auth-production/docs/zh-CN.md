...
> Lesson 16 在内存中搭建了 OAuth 2.1 状态机。到 2026 年，每个交付给真实组织的 MCP server 都会位于生产 auth 之后：能扩展到无界 client population 的 client enrollment（Client ID Metadata Documents 优先，dynamic client registration 作为向后兼容 fallback）、authorization-server metadata discovery（RFC 8414 *或* OpenID Connect Discovery）、不会在凌晨 3 点 token validation 中断的 JWKS cache refresh，以及拒绝 cross-resource replay 的 audience-pinned tokens。本课用三个角色建模完整表面：authorization server、resource server（MCP server）和 client，让你追踪从 discovery 到已验证 tool call 的每一次跳转。
>
> **Spec note (2025-11-25)：** 2025 年 11 月 MCP authorization spec 将 Dynamic Client Registration 从 `SHOULD` 降级为 `MAY`，并把 **Client ID Metadata Documents (CIMD)** 设为推荐的默认 enrollment 机制。本课按 spec 优先级同时教授两者；代码为了 walkthrough 完全自包含在一个进程里，所以保留 DCR。

**类型：** 构建
**语言：** Python（stdlib）
**先修：** Phase 13 · 16（OAuth 2.1 state machine）、Phase 13 · 17（gateways）
**时间：** 约 90 分钟

## 学习目标

- 通过 RFC 8414 metadata 发现 authorization server 并验证契约。
- 实现 RFC 7591 dynamic client registration，让 MCP clients 无需管理员介入即可 enroll。
- 按计划缓存并刷新 JWKS keys，使 signature verification 能承受 key roll-over。
- 使用 RFC 8707 resource indicators 将 tokens 固定到单个 MCP resource，并拒绝 confused-deputy reuse。
- 清晰分离三个角色：authorization server、resource server、client，让每个角色只执行属于自己的检查。
- 阅读 IdP capability matrix，并在 IdP 不能满足 MCP auth profile 时拒绝部署。

## 要解决的问题

Lesson 16 simulator 在内存中运行 OAuth 2.1。生产环境有三个只靠内存 simulator 看不到的运维缺口。

第一个缺口是 enrollment。真实组织会运行数百个 MCP servers 和数千个 MCP clients。运营人员不会把每个 Cursor 用户手工注册成 OAuth client。2025-11-25 spec 为 clients 给出了解决这个问题的优先顺序：如果已有 pre-registered `client_id`，就使用它；否则使用 **Client ID Metadata Document**（client 用自己控制的 HTTPS URL 标识自己，authorization server *pulls* metadata）；否则回退到 **RFC 7591 dynamic client registration**（client *pushes* 一个 `POST /register`，并当场收到 `client_id`）；否则提示用户。CIMD 是推荐默认值，因为它在保留 DNS-rooted trust model 的同时，完全移除 per-server registration；DCR 保留用于向后兼容。二者都从 authorization server metadata 中发现入口点：CIMD 用 `client_id_metadata_document_supported`，DCR 用 `registration_endpoint`。

第二个缺口是 key rotation。JWT validation 依赖 authorization server 的 signing keys，这些 keys 以 JSON Web Key Set (JWKS) 发布。authorization server 会按计划 rotate 这些 keys（通常每小时一次，事故响应时可能更快）。MCP server 如果只在启动时获取一次 JWKS，在 rotation window 之前验证正常，之后所有请求都会失败，直到重启。生产环境把 JWKS 接成带 refresh job 的缓存值：在旧 keys 过期前覆盖缓存；并在 cache miss 时做一次 fallback fetch，用于处理由比缓存更新的 key 签名的 token 到达的情况。

第三个缺口是 audience binding。Lesson 16 引入了 RFC 8707 resource indicators。在生产中，该 indicator 会变成每次请求上的硬 claim check。MCP server 将 `token.aud` 与自身 canonical resource URL 比较，不匹配就以 HTTP 401 拒绝。这是抵御同一 trust mesh 中的 upstream MCP server（或持有某一服务器 token 的恶意 client）把 token replay 到另一台服务器的唯一防线。

本课把每个缺口映射到 surface 上的具体部件。metadata document 是一个 HTTP endpoint。JWKS cache refresh 是 scheduled job 加 key-value cache。JWT validation 是 resource server 在 dispatch 任何 tool 前运行的 routine。保持三个角色分离，每个角色只执行它拥有的检查：authorization server 签发并 rotate keys，resource server 缓存并验证，client 发现并 enroll。
...
给定 MCP resource URL 的 client 会链式 discovery：RFC 9728 的 `oauth-protected-resource`（resource server 文档）命名 issuer，然后 `oauth-authorization-server`（本 RFC）命名每个 endpoint。client 永远不会 hard-code authorization URL。
...
- 至少公布一种 enrollment path：`client_id_metadata_document_supported: true`（CIMD，优先）**或** `registration_endpoint`（RFC 7591 DCR，fallback）。任一种都满足契约；你不再硬性要求 DCR。
- `response_types_supported` 对 OAuth 2.1 来说恰好是 `["code"]`。

如果缺少 `S256`，MCP server 会拒绝部署到这个 IdP；PKCE 没有降级模式。如果*两种* enrollment path 都没有公布，且你没有 pre-registered `client_id`，也无法 enroll；这是 deployment manifest 错误，不是代码错误。

### RFC 9728（回顾）：Protected Resource Metadata

Lesson 16 已覆盖 RFC 9728。生产环境中的增量是：这个文档是 client 寻找被*此* MCP server 信任的 authorization servers 的唯一位置。单个 MCP server 可以接受来自多个 IdP 的 tokens（一个给员工，一个给合作伙伴）。RFC 9728 声明这个集合；RFC 8414 说明每个 IdP 支持什么。
...
- **SSRF。** authorization server 会获取 attacker-supplied URL。它必须防御 server-side request forgery（不能 fetch 内部/admin endpoints）。
- **localhost impersonation。** 仅 CIMD 无法阻止本地攻击者声称一个合法 client 的 metadata URL 并绑定任意 `localhost` redirect。authorization server **MUST** 在 consent 时清晰展示 redirect URI hostname，并且 **SHOULD** 对 `localhost`-only redirects 发出警告。
...
DCR 现在是 `MAY`，保留用于向后兼容 2025-11-25 之前的部署，以及尚不支持 CIMD 的 IdPs。没有它（也没有 CIMD 或 pre-registration）时，每个 MCP client（Cursor、Claude Desktop、自定义 agent）都需要和 IdP admin 进行带外交换。有了 DCR，client 会 post：
...
`token_endpoint_auth_method: none` 是在用户设备上运行的 MCP clients 的正确默认值。它们只获得 `client_id`，没有可被 exfiltrate 的 `client_secret`。PKCE 提供 public clients 需要的 proof-of-possession。

三个生产陷阱：

- registration endpoint 必须按 source IP 做 rate-limit。否则 hostile actor 可以脚本化数百万个 fake registrations，耗尽 `client_id` namespace。在 registrar 处理请求前先运行 rate-limit check。
...
- client（不是 server）防御 **mix-up attacks**：它在 redirect 前记录 expected `issuer`，并在 redeem code 前验证 `iss` authorization-response 参数（RFC 9207）。PKCE alone 不能阻止 mix-up，因为 client 会把自己的 `code_verifier` 交给被引导到的任何 token endpoint。

OAuth 2.1 draft 是底层；RFC 8414/7591/8707/9728/9207 + RFC 7636 + CIMD 是表面；MCP spec 是 profile。

### IdP capability matrix

不是每个 IdP 都支持完整 MCP profile。下表记录的是截至 2025-11-25 spec 的事实能力声明。它是*部署 gate*，不是推荐。
...
| Enterprise SSO (Microsoft Entra ID) | yes | emerging | yes (premium tiers) | yes | yes | DCR availability 随 tenant tier 不同；部署前在目标 tenant 验证。 |
| Enterprise SSO (Okta) | yes | emerging | yes (Okta CIC / Auth0) | yes | yes | Auth0（现在是 Okta CIC）支持 DCR；classic Okta orgs 需要 admin pre-registration。 |
| Social login IdPs (generic) | varies | no | rarely | rarely | yes | 大多数 social IdPs 把 clients 当作 static partners；没有 self-service enrollment。只把它用作 identity source，在上层叠加自己的 MCP-aware authorization server。 |
| Custom / homegrown | depends | depends | depends | depends | depends | 如果自己实现，就实现完整 profile 并优先 CIMD。跳过 PKCE 或 audience binding 会破坏 MCP auth contract。 |

deployment manifest 的拒绝规则：如果所选 IdP 没有在 `code_challenge_methods_supported` 中列出 `S256`，MCP server 拒绝启动，因为 PKCE 没有降级模式。Enrollment 是较软的 gate：你需要*一种*可用路径（pre-registered `client_id`、`client_id_metadata_document_supported: true` 或 `registration_endpoint`）。单独缺少 DCR 不再触发拒绝，因为 CIMD 或 pre-registration 可以覆盖。

### JWKS refresh pattern（AS rotate，resource server refresh）
...
PKCE alone 不能阻止 mix-up，因为 client 会把自己的 `code_verifier` 交给被引导到的任意 token endpoint。这就是 spec 为什么把 issuer 与 PKCE verifier 和 `state` 一起按请求记录。
...
2. MCP client 调用 metadata endpoint，并检查其 enrollment options（CIMD 的 `client_id_metadata_document_supported`、DCR 的 `registration_endpoint`）和 `S256` PKCE support。
...
本课产出 `outputs/skill-mcp-auth.md`。给定 MCP server config 和 IdP capability set，该 skill 会发出需要搭建的 auth surface：protected-resource metadata、要使用的 enrollment path（CIMD、pre-registration 或 DCR fallback）、JWKS refresh schedule、scope mapping，以及 IdP 不支持完整 RFC profile 时要应用的 refusal rules。

## 练习

1. 运行 `code/main.py`。追踪 flow。注意 IdP 如何在 step 6 rotate key，scheduled `refresh_jwks` 如何重新拉取已发布集合，以及 old token（overlap window）和 fresh token 都如何无需重启即可验证。
...
3. 向 `register_client` 添加一个 rate-limit check，并确保它在 registrar 接受请求之前运行。使用一个以 IP 为 key 的小 dict 保存 per source IP token-bucket。

4. 阅读 RFC 7591，找出本课 `/register` handler 没有验证的两个字段。添加验证。（提示：`software_statement` 和 `redirect_uris` URI scheme。）

5. 添加 Client ID Metadata Document path。提供一个 `client_id` 等于自身 URL 的 `client.json`，并让 authorization server fetch 并验证它（若 `client_id` ≠ URL 则拒绝）。确认 CIMD client 无需 `register_client` call 即可 enroll。
...
| DCR | “Self-service client registration” | RFC 7591 `POST /register` flow；在 2025-11-25 被降级为 `MAY` fallback |
| JWKS | “Public keys for JWT validation” | JSON Web Key Set，从 `jwks_uri` 获取，并以 `kid` 索引 |
| Rotate vs refresh | “Updating the keys” | *Rotate* = AS mint/retire signing keys；*refresh* = resource server 重新获取已发布集合。resource servers 只会 refresh |
| Resource indicator | “Audience parameter” | RFC 8707 `resource` 参数，将 token 固定到一个 server |
| `aud` claim | “Audience” | validator 与 canonical resource URL 比较的 JWT claim |
| Audience replay | “Token replay” | 签发给 Server A 的 token 被提交给 Server B；通过 audience validation 防御（spec：access-token privilege restriction） |
| Confused deputy | “Proxy token misuse” | 使用 static client ID 的 MCP proxy 在没有 per-client consent 时转发 token；不同于 audience replay |
| Mix-up attack | “Wrong token endpoint” | client 被引导到攻击者 endpoint 去 redeem 诚实 AS 的 code；通过 client-side RFC 9207 `iss` 防御 |
| `iss` allow-list | “Trusted authorization servers” | protected-resource metadata 的 `authorization_servers` 命名的集合 |
| `resource_metadata` | “Where to find the PRM doc” | 401/403 上命名 RFC 9728 metadata URL 的 `WWW-Authenticate` 参数 |
| Public client | “Native or browser client” | 没有 `client_secret` 的 OAuth client；PKCE 作补偿 |
| `WWW-Authenticate` | “401/403 response header” | 携带驱动 client recovery 的 `Bearer error=...` 指令 |
...
- [MCP blog — One Year of MCP: November 2025 Spec Release](https://blog.modelcontextprotocol.io/posts/2025-11-25-first-mcp-anniversary/) — 2025-11-25 中的变化（CIMD、XAA、DCR demotion）
