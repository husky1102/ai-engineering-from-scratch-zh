...
- 对 CPU 与 GPU 上的矩阵乘法做基准测试，并测量加速比
- 使用 fp16 经验法则估算你的 VRAM 能容纳的最大模型

## 要解决的问题

第 1-3 阶段的大多数课程在 CPU 上都能顺利运行。但当你开始训练 CNN、transformer 或 LLM（第 4 阶段及以后）时，就需要 GPU 加速。一次在 CPU 上需要 8 小时的训练，在 GPU 上可能只需要 10 分钟。
...
大多数课程都可以在 CPU 上完成。需要 GPU 的课程会明确说明，并附上 Colab 链接。
...
2. 如果你没有 GPU，就在 Google Colab 上运行它并进行比较
3. 检查你的 GPU 显存容量，并估算它能容纳的最大模型（经验法则：fp16 每个参数占 2 字节）
...
| VRAM | “GPU 显存” | GPU 上的视频 RAM，独立于系统 RAM。它会限制模型大小。 |
| fp16 | “半精度” | 16 位浮点数，占用的内存是 fp32 的一半，精度损失很小 |
