...
graph TD
    ...
    subgraph with_docker["使用 Docker：同一个镜像到处运行"]
...
| Container（容器） | 镜像的运行中实例。你的厨房。 |
| Dockerfile | 构建镜像的指令。逐层构建。 |
| Volume（卷） | 能在容器重启后继续存在的持久化存储。 |
...
nvidia/cuda:12.4.1-devel-ubuntu22.04
  ...
  适用：构建需要 nvcc 的包（flash-attn、bitsandbytes）
  大小：~4 GB
...
| 术语 | 常见说法 | 实际含义 |
|------|----------|----------|
| Container（容器） | “轻量 VM” | 使用宿主机内核的隔离进程，拥有自己的文件系统和网络 |
| Image layer（镜像层） | “缓存步骤” | 每条 Dockerfile 指令都会创建一层。未变化的层会被缓存，所以重建很快。 |
| NVIDIA Container Toolkit | “Docker 里的 GPU” | 通过 `--gpus` 标志把宿主机 GPU 暴露给容器的运行时钩子 |
| Volume mount（卷挂载） | “共享文件夹” | 映射进容器的宿主机目录。容器停止后，变更仍然保留。 |
| Base image（基础镜像） | “起点” | Dockerfile 基于其构建的 `FROM` 镜像。它决定了预装内容。 |
